...
const Admin = () => {
  ...
  useEffect(() => {
    if (loginStatus === "HR"){
      axios.get(`${baseUrl}hrdetails`)
      .then(res => setHRData(res.data))
      .catch(err => {
        alert(err);
      });
    }else if (loginStatus === "ADMIN"){
      axios.get(`${baseUrl}admindata`)
      .then(res => setHRData(res.data))
      .catch(err => {
        alert(err);
      });
  ...
  //GET INTERVIEW DATA
  useEffect(() => {
    axios.get(`${baseUrl}interviewdata`)
      .then(res =>{
        let conData = []
  ...
  //GET ACCEPTED DATA
  useEffect(() => {
    axios.get(`${baseUrl}offeracpdata`)
      .then(res =>{
        let conData = []
        for (let dbData of res.data){
          if (dbData.accesscode === token){
...
//////////////////////////////////////////////////////////////////////////////////////
  ...
  const onClickIntDelete = async (id) => {
    try{
      await axios.delete(`${baseUrl}interviewdata/${id}`)
      window.location.reload()
    }catch(err){
      alert(err)
    }
  ...
  const onClickOffDelete = async (id) => {
    try{
      await axios.delete(`${baseUrl}offerdata/${id}`)
      window.location.reload()
    }catch(err){
      alert(err)
    }
  ...
  const onClickOnboardingDelete = async (id) => {
    try{
      await axios.delete(`${baseUrl}onboarding/${id}`)
      window.location.reload()
    }catch(err){
      alert(err)
    }
...
//////////////////////////////////////////////////////////////////////////////////////
  ...
  const onClickAddToInterviewSub = (id) => {
    ...
    console.log(phoneno)
    axios.post(`${baseUrl}interviewdata` , {accesscode, name, position, experience, location, salary, email, phoneno, about, address})
    .then(res => {
  ...
  const onClickAddToAcceptSub = async (id) => {
    ...
    await axios.post(`${baseUrl}offeracpdata` , {accesscode, name, position, experience, location, salary, email, phoneno, about, address})
    .then(res => {
  ...
  const onClickAddToOnboardingSub = async (id) => {
    ...
    await axios.post(`${baseUrl}onboarding` , {accesscode, name, position, experience, location, salary, email, phoneno, about, address})
    .then(res => {
      alert("Successfully Added")
      window.location.reload()
    })
  ...
  const onClickAddToEmployeSub = async (id) => {
    ...
    await axios.post(`${baseUrl}employedata` , {accesscode, name, position, experience, location, salary, email, phoneno, about, address, password, role})
    .then(res => {
      alert("Successfully Added")
      window.location.reload()
    })
  ...
  const handleSendEmail = () => {
    axios.post(`${baseUrl}send-email`, emailData)
      .then(response => {
